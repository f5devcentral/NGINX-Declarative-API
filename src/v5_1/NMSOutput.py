...
def NMSOutput(d, declaration: ConfigDeclaration, apiversion: str, b64HttpConf: str,
    ...
    nmsUsername = v5_1.MiscUtils.getDictKey(d, 'output.nms.username')
    nmsPassword = v5_1.MiscUtils.getDictKey(d, 'output.nms.password')
    nmsInstanceGroup = v5_1.MiscUtils.getDictKey(d, 'output.nms.instancegroup')
    nmsSynctime = v5_1.MiscUtils.getDictKey(d, 'output.nms.synctime')

    nmsUrlFromJson = v5_1.MiscUtils.getDictKey(d, 'output.nms.url')
    ...
    d_policies = v5_1.MiscUtils.getDictKey(d, 'output.nms.policies')
    if d_policies is not None:
        for policy in d_policies:
            if 'versions' in policy:
                for policyVersion in policy['versions']:
                    status, content = v5_1.GitOps.getObjectFromRepo(object=policyVersion['contents'],
    ...
    d_certs = v5_1.MiscUtils.getDictKey(d, 'output.nms.certificates')
    ...
    d_certificates = v5_1.MiscUtils.getDictKey(d, 'output.nms.certificates')
    ...
    nginxMainConf = j2_env.get_template(NcgConfig.config['templates']['nginxmain']).render(
        nginxconf={'modules': v5_1.MiscUtils.getDictKey(d, 'output.nms.modules')})
    ...
    if currentBaseStagedConfig is not None and newBaseStagedConfig == currentBaseStagedConfig:
        ...
        igUid = v5_1.NIMUtils.getNIMInstanceGroupUid(nmsUrl=nmsUrl, nmsUsername=nmsUsername,
        ...
        if len(checkJson['details']['failure']) > 0:
            ...
            if nmsSynctime == 0:
                ...
                job = schedule.every(nmsSynctime).seconds.do(lambda: V5_1_CreateConfig.configautosync(configUid))
