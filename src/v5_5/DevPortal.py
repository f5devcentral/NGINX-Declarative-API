...
def createDevPortal(locationDeclaration: dict, authProfiles: Authentication={}):
    if locationDeclaration['apigateway']['openapi_schema']:
        status, apiSchemaString = v5_5.GitOps.getObjectFromRepo(
        ...
        if v5_5.MiscUtils.yaml_or_json(apiSchemaString['content']) == 'yaml':
            ...
            status, devportalJSON = buildDevPortal(openapischema = v5_5.MiscUtils.yaml_to_json(apiSchemaString['content']))
        else:
